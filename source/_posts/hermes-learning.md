...
通过这样的自我修订防止跳入循环的skill竟然也能自动化创建了，这个skill的亮点就是让ai拥有科学探究解决问题的能力，在最少试错次数中获得最大化回收效益，同时保证了输出质量，也确认不会犯**记忆丢失，问牛答马**式错误，ai的自检能力得到深化。
...
2. **隐式触发 — 系统 prompt 里的规则**
- 每次对话开始的系统提示词（*system prompt*）里都有写：
  ``` md 
    If a skill matches or is even partially relevant to your task, you MUST load it with skill_view(name) and
...
ppt制作是每一个上班族和教师的头疼之处，既要保证信息密度，又要保证布局与配色的着调，对于一个没有美术功底的人，无异于登天，html-ppt-skill是一个多平台skill，可以使得agent能够成为一个专业的ppt生成师：

结合最近的gpt-image2的爆火，我们指定它作为宣传ppt对象，在一段时间的自动部署skill和制定计划执行之后，hermes输出了结果，同时将这个整合为一个新的skill：

![hermes-ppt](/images/hermes-ppt.png)

下面是一部分制作的ppt，省去了一些内容大致相同以及目录页:    
...
- **如果有必要，你也可以参考[ppt-url](https://github.com/lewislulu/html-ppt-skill/)来自己动手**


### 结语
---
hermes的体验准确来说是一周半，从5/1到5/11中间几天在学校，体验感着实不错。

- 我会在下一篇文章补充hermes的其他生态与工作流搭配。
---
- hermes作为agent开源框架之光，是你优化工作流的极佳选择。
- 我会借hermes的上下文机制，在以后的文章中阐述分析上下文工程，
- 同时也会引出对windowswsl2的局限性问题，到时候我还会做一些有趣的小项目，这也许会在暑假前完成。敬请期待。